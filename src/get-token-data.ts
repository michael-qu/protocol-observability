...
async function GetTokenData(
  ...
  const protocolDataProviderContract = CreateContractInstance(
    rpcUrl,
    abi,
    address,
  );
  const reserveTokenAddresses: ReserveTokenAddress[] =
    await protocolDataProviderContract.methods.getAllReservesTokens().call();
  const tokenAddresses: TokenData[] = [];

  for (const rta of reserveTokenAddresses) {
    const tokenData: TokenData = await protocolDataProviderContract.methods
      .getReserveTokensAddresses(rta.tokenAddress)
      .call();
    ...
    tokenData.price = await getAssetPrice(
      modeConstants.rpcUrl,
      ironcladAddresses.Oracle,
      oracleAbi,
      rta.tokenAddress,
    );
...
async function getAssetPrice(
  ...
  const BASE_CURRENCY_UNIT: bigint = await oracleContract.methods
    .BASE_CURRENCY_UNIT()
    .call();
  const price: bigint = await oracleContract.methods
  ...
  return (
    Number((price * precisionFactor) / BASE_CURRENCY_UNIT) /
